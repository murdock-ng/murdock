...
main() {
    ...
    local murdock_api_url=${CI_BASE_URL}
    if [ -n "${CI_DOCKER_API_URL}" ]; then
        echo "Running in Docker, use container API URL: ${CI_DOCKER_API_URL}"
        murdock_api_url=${CI_DOCKER_API_URL}
    fi
